...
3.  Ведущий производит согласование параметров, сообщая гостям о
    ...
    [Подключение-при-выполнении](#Подключение-при-выполнении))
...
4.  Если гость не может экспортировать требуемые параметры, он сообщает
    ...
    параметров. [Общий формат сообщения начиная с шага инициализации](#Общий-формат-сообщения-начиная-с-шага-инициализации)
...
1.  Когда гость завершает очередной шаг моделирования (или
    инициализацию) и находится на временном шаге *t<sub>i</sub>*, он должен
    оценить размер своего следующего шага *Δt*.
...
3.  Получив все оценки шагов от гостей, ведущий выбирает наименьший
    *Δt<sub>min</sub>* и рассылает гостям это
    значение.

4.  Гости должны совершить шаг, соответствующий полученному значению
    *Δt<sub>min</sub>*, либо интерполировать полученные результаты для
    момента времени *t=t<sub>i</sub>+Δt<sub>min</sub>*
...
6.  Ведущий, получив значения рассчитанных параметров для момента
    времени *t=t<sub>i</sub>+Δt<sub>min</sub>*, передает эти данные запросившим их
...
или
(6)
основного цикла, ведущий, вместо целевого шага *Δt<sub>min</sub>*,
...
## Общий формат сообщения начиная с шага инициализации
